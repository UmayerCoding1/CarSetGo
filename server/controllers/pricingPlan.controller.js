...
export const getFreePlan = async (req, res) => {
  ...
  try {
    ...
    if (user.plan && user.plan === "free") {
      return res
        .status(400)
        .json({ message: "You are already on the free plan", success: false });
    }
    const updatedUser = await User.findByIdAndUpdate(
      userId,
      { plan: name, planDetails: freePlan._id, isPlanActive: true },
      { new: true }
    );
    res
      .status(200)
...
export const getAllPricingPlans = async (req, res) => {
  try {
    const pricingPlans = await PricingPlan.find().select(
      "-__v -createdAt -updatedAt"
    );
    res
      .status(200)
      .json({ message: "Pricing plans fetched successfully", pricingPlans });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
