    ...
    <div className="p-4 max-h-screen overflow-auto scrollb">
      ...
      <div className="bg-white rounded-xl shadow border border-gray-100 overflow-x-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {users.map((user, idx) => (
              <tr key={user._id} className={`border-b last:border-b-0 ${user?.role !== 'blacklisted' ? 'bg-red-50' : 'hover:bg-blue-50'} transition`}>
                <td className="py-2 px-4">{(page - 1) * usersPerPage + idx + 1}</td>
                <td className="py-2 px-4 font-medium text-gray-900">{user.fullname}</td>
                <td className="py-2 px-4">{user.email}</td>
                <td className="py-2 px-4">
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${user.role === 'admin' ? 'bg-blue-200 text-blue-800' : user.role === 'seller' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>{user.role}</span>
                ...
                <td className='py-2 px-4'>
                    ...
                    //   setUsers(prev => prev.map(u => u.id === user.id ? { ...u, role: newRole } : u));
                      // TODO: Call API to update user role in backend
                ...
                <td className='py-2 px-4 flex items-center justify-center' >
                   <div className='flex items-center gap-2'>
                     <button className='flex items-center gap-2 bg-red-500 text-white px-2 py-1 rounded text-xs'>
                        <Ban size={13}/> Block
                    </button>
                    <button className='flex items-center gap-2 bg-red-500 text-white px-2 py-1 rounded text-xs'>
                        <Ban size={13}/> Block
